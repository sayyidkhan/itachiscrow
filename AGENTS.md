...
- The voice console uses the pinned `thinking-orbs/engine` canvas renderer: edit `src/voice-orb.js` and `dist/voice-orb.css`; `npm run build` regenerates tracked `dist/voice-orb.js` and its MIT licence for both deployments. It includes all nine shapes plus Auto, with only the visual preference in localStorage. `CrowLive` meters existing input/output streams through optional `onLevel`, forwarded as `crow:voice-level`; never request another microphone stream or connect the meter to speaker output. Release meter resources on stop, failure and cancellation. Animation pauses for hidden overlays/tabs and reduced motion. `npm run verify:voice-orb` covers six viewports with simulated voice and a real local Web Audio signal, not a live provider call.
