...
- Chat and voice captions must leave steering accessible at every viewport. `steering.css` reserves a lower control area on taller phones and uses a side-by-side layout on short screens. Held input sends a 100 ms heartbeat independent of rendering; preserve the engine's lost-input timeout and second-finger button handling. The joystick's circular Roll button rotates all three GLB parts around their shared pivot with a level camera; cancellation restores an upright pose and reduced motion skips the roll. Speed cycles 1×/2×/3× for manual and directional voice translation, leaving turn rate, destination journeys and orbit timing unchanged.
