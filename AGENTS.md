...
- The voice console uses the pinned `thinking-orbs/engine` canvas renderer: edit `src/voice-orb.js` and `dist/voice-orb.css`; `npm run build` regenerates tracked `dist/voice-orb.js` and its MIT licence for both deployments. The Orb style picker includes all nine shapes plus Adaptive (stored as `auto`), with only the visual preference in localStorage. Show only the latest caption over the map; full history stays in Chat. Closed steering uses the measured `--voice-panel-height` to sit above the lowered voice panel on phones. `CrowLive` meters existing input/output streams through optional `onLevel`, forwarded as `crow:voice-level`; never request another microphone stream or connect the meter to speaker output. Release meter resources on stop, failure and cancellation. Animation pauses for hidden overlays/tabs and reduced motion. `npm run verify:voice-orb` covers six viewports with simulated voice and a real local Web Audio signal, not a live provider call.
...
- Discovery artwork lives in `dist/images/destinations/` as public, bundled WebP illustrations. Keep their module-relative paths compatible with both `/crow/` and GPT's root deployment. Local/day reuse is thematic artwork, not venue photography. Nearby must use Google venue photos or a neutral missing-photo placeholder, never curated landmark illustrations. Preserve Google photo credits. Artwork provenance and generation prompts are in `docs/discovery-artwork.md`.
...
- `mobile-ui.js` places the live journey card inside open chat or voice captions; closing these restores its map position below flight controls. Keep progress in normal panel flow so it cannot obscure chat controls or messages, including short keyboard viewports.
...
- Text chat and steering are mutually exclusive at every viewport: `crow:text-chat-open` closes and stops the joystick; `crow:steering-open` closes chat while preserving drafts/history. Voice captions and calls remain available with steering. `steering.css` reserves space for voice on taller phones and places it beside steering on short screens. The joystick and Roll unlock only after explicit Lift off; the steering panel exposes Lift off / Stop lift off from the first perch. Takeoff uses wall-clock duration, a fixed surface elevation and a matching end camera for the transition into steering. Steering retains that absolute elevation, eases input and updates camera properties directly; never switch back to mesh-relative placement or restart native camera animations every steering frame. Held input sends a 100 ms heartbeat independent of rendering; preserve the lost-input timeout and second-finger button handling. Roll rotates all three GLB parts around their shared pivot with a level camera; cancellation restores an upright pose and reduced motion skips the roll. Speed cycles 1×/2×/3× for manual and directional voice translation, leaving turn rate, destination journeys and orbit timing unchanged.
