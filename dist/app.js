...
function flightPosition(s){
 ...
 altitude:57+2.2*Math.sin(s/31)+.22*Math.sin(flightTime*2*Math.PI*1.6)};
}
...
function camera(s){
 ...
 heading:flightBearing(s),tilt:high?52:73,range:high?90:46,roll:0};
}
function poseCrow(s){
...
async function mountCrow(Model){
 ...
 // Verify actual geometry downloads before enabling flight. Do not silently fall back to a sprite.
 await Promise.all(names.map(async name=>{
  const response=await fetch(new URL('models/'+name+'.glb',document.baseURI));
 ...
 crowParts=names.map(name=>new Model({src:new URL('models/'+name+'.glb',document.baseURI).href,
  position:flightPosition(0),altitudeMode:'ABSOLUTE',scale:2.2}));
 crowHeading=flightBearing(0);poseCrow(0);
 crowParts.forEach(part=>map.append(part));
}
function hint(s){$('hint').textContent=s;}
function status(s){$('status').textContent=s;}
function stop(){playing=false;transitioning=false;cancelAnimationFrame(frameId);map?.stopCameraAnimation?.();$('fly').innerHTML='Resume flight <span aria-hidden="true">↗</span>';status('Paused · explore the map');}
function draw(t){if(!playing)return;const dt=Math.min((t-last)/1000,.1);last=t;flightTime+=dt;progress+=dt*speed;if(progress>=total){progress=total;stop();$('fly').textContent='Fly again';status('Flight complete');hint('You’ve scouted the block. Tap Places to explore what’s nearby.');$('progress').style.width='100%';$('progress-text').textContent='100%';return;}
 ...
 map.center=cam.center;map.heading=heading;map.tilt=cam.tilt;map.range=cam.range;
