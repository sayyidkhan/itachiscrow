...
Choose **Steer** at the bottom left to open the glass joystick. Hold and drag up to fly forward, down to reverse, and left/right to turn; diagonal movement turns while flying. Release to hover. When perched, keep holding through takeoff. With the joystick focused, arrow keys steer, Page Up/Page Down change height, and Space or Escape stops. The control stops on lost focus or when you switch tabs. Chat and voice captions leave room for steering on phones too; short screens place them alongside the joystick. Your draft and voice call stay open while you steer.

The circular **Roll** button makes the 3D crow do one barrel roll, returning upright while the camera stays level. It lifts off first if perched; Stop or another flight action cancels it. Reduced-motion settings skip the roll. The circular **1×** button cycles steering speed through **2×**, **3×**, then **1×**. It changes forward, reverse and height movement, including directional voice commands; turning speed stays steady. Speed lasts for the current page session and does not change destination journeys or landmark orbits.
