...
test('Sites adapter preserves assets, configuration, validation and provider contracts',async()=>{
  ...
  assert.equal(await (await worker.fetch(request('/config.js'),env)).text(),'window.CROW_MAPS_KEY="test-browser-key";window.CROW_MAPS_FALLBACK_KEY="";window.CROW_MAPS_FALLBACK_KEY_2="";');
  const thirdConfig=await (await worker.fetch(request('/config.js'),{...env,CROW_MAPS_FALLBACK_KEY_2:'third-test-key'})).text();
  assert.ok(thirdConfig.includes('window.CROW_MAPS_FALLBACK_KEY_2="third-test-key"'));
