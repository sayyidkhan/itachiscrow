...
export function createHandler({ env = process.env, fetchImpl = globalThis.fetch, distDir = resolve(ROOT, 'dist'), config = createConfig(env), now = Date.now } = {}) {
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const handlers = { '/api/chat':commandChat, '/api/portrait': generatePortrait, '/api/discover': discoverOffers, '/api/panorama': generatePanorama, '/api/plan': generatePlan, '/api/live/session': createLiveSession };
