...
test('private config supplies settings while environment overrides remain compatible',async t=>{
 ...
 await writeFile(join(root,'config.json'),JSON.stringify({OPENAI_API_KEY:'private-fixture',OPENAI_TEXT_MODEL:'file-model',PORT:8806,APP_BASE_PATH:'/crow'}));
 await writeFile(join(root,'.env'),'CROW_MAPS_KEY1=test-map\nPUBLIC_ORIGIN=https://crow.example\n');
...
test('Worker builds exclude local config unless explicitly selected and never publish it as an asset',async t=>{
 ...
 await writeFile(join(root,'config.json'),JSON.stringify({OPENAI_API_KEY:'build-private-fixture',OPENAI_TEXT_MODEL:'configured-model'}));
 ...
 const status=await worker.fetch(new Request('https://crow.example/api/status'),env);
 const text=await status.text();assert.ok(!text.includes('build-private-fixture'));
 assert.equal(JSON.parse(text).capabilities.chat,true);assert.equal(JSON.parse(text).openai.planModel,'configured-model');
 const browser=await (await worker.fetch(new Request('https://crow.example/config.js'),env)).text();assert.ok(!browser.includes('build-private-fixture'));
